...
export const WIKI_PREAMBLE = [
  ...
  "  b. `read-raw-sources.ts` (full bundle — no args) → load **all** wish categories (`learn`, `place`, `goal`) and cross-check **activity**: plan_pages, marked, skills, todos, wishlist_todos, invoices (+ clients), mind_map (scenes + life_nodes). Identify stale or missing wiki syntheses from that evidence.",
  ...
  "  e. Build the bento card list (≤9) as an **eagle view toward the user's wishes**: start from wishes, then alignment vs actual behaviour, then actionable keep/stop guidance. Exactly one `heartbeat` card. Every card MUST have `wikiSlug` and a stable **`slot`** (see SKILL.md canonical slots: wishes_compass, alignment, keep_doing, stop_doing, heartbeat, signals, …). Use `slot` instead of random `id` so each run **updates** the same dashboard rows.",
